                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                ...
                                <input type="text" name="code" value="{{ old('code', $tax->code) }}" required
                                       class="w-full bg-black/30 border border-white/10 rounded-xl px-4 py-2.5 text-xs font-black text-white focus:outline-none focus:border-yellow-500/50 transition-colors uppercase font-mono">
                            </div>
                            <div class="md:col-span-2">
                                ...
                                <input type="text" name="name" value="{{ old('name', $tax->name) }}" required
                                       class="w-full bg-black/30 border border-white/10 rounded-xl px-4 py-2.5 text-xs font-bold text-white focus:outline-none focus:border-yellow-500/50 transition-colors uppercase">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select name="type" class="w-full appearance-none bg-black/30 border border-white/10 rounded-xl px-4 py-2.5 text-xs font-bold text-white focus:outline-none focus:border-yellow-500/50 transition-colors cursor-pointer uppercase">
                                    <option value="iva" {{ (old('type', $tax->type)) == 'iva' ? 'selected' : '' }}>IVA (Bienes y Servicios)</option>
                                    <option value="inc" {{ (old('type', $tax->type)) == 'inc' ? 'selected' : '' }}>INC (Consumo)</option>
                                    <option value="retefuente" {{ (old('type', $tax->type)) == 'retefuente' ? 'selected' : '' }}>Retención en la Fuente</option>
                                    <option value="reteiva" {{ (old('type', $tax->type)) == 'reteiva' ? 'selected' : '' }}>Reteiva</option>
                                    <option value="reteica" {{ (old('type', $tax->type)) == 'reteica' ? 'selected' : '' }}>Reteica</option>
                                </select>
                            </div>
                        </div>
